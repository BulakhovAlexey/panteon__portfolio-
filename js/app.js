(() => {
 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
    ...
    (() => {
        ...
        let gotoBlock = (targetBlock, noHeader = false, speed = 800, offsetTop = 0) => {
        ...
        let formValidate = {
            ...
            nameTest(formRequiredItem) {
                return !/^([А-Я]{1}[а-яё]{1,23}|[A-Z]{1}[a-z]{1,23})$/.test(formRequiredItem.value);
            }
        };
        function formSubmit() {
        ...
        function setClass(tableElement, dateElement, rowElements, toggle = true) {
            if (toggle) rowElements[1].classList.add("_active"); else rowElements[1].classList.remove("_active");
            let currentIndex = 0;
            rowElements.forEach(((item, index) => {
                if (dateElement.innerText === item.innerText) currentIndex = index;
            }));
            let head = tableElement.querySelector(".subscription-table__head");
            let item = head.querySelectorAll("td");
            if (toggle) item[Math.floor(currentIndex / 2)].classList.add("_active"); else item[Math.floor(currentIndex / 2)].classList.remove("_active");
        }
        function listenTable(table) {
            table.addEventListener("mouseover", (e => {
                ...
                if (e.target.closest(".subscription-table__head")) return false; else {
