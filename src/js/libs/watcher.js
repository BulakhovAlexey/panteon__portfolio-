...
//если нужно добавлять дата атрибуты динамически - раскоментировать вызов функции и
//вписать название класса кому передваем
function addWatcherForItems(className) {
  let items = document.querySelectorAll(className);
  items.forEach((element) => {
    element.dataset.watch = null; // добваление data-watch
    //element.dataset.watchMargin = "20"; // добваление data-watch margin px or %
    element.dataset.watchThreshold = "0.9"; // добваление data-watch-threshold from 0 to 1
    //element.dataset.watchOnce = null; // добваление data-watch-once
  });
}
//addWatcherForItems("className");  //".exampleClassName"
